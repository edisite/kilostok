...
class PermintaanPembelian extends Admin_Controller {
	...
	$select = '*';
		...
		$where_like['data'][] = array(
			'column' => 'project_nama, permintaan_pembelian_nomor, permintaan_pembelian_jenis_nama, permintaan_pembelian_tanggal, permintaan_pembelian_status_nama',
			...
			foreach ($query->result() as $val) {
				...
				if ($type == 1) {
					if ($val->permintaan_pembelian_status > 4) {
						$button = $button.'<a href="'.base_url().'Pembelian/Surat-Permintaan-Pembelian/EditForm/'.$val->permintaan_pembelian_id.'">
